...
pub fn camera_look_system(demo: &mut LevelDemo, world: &mut World) {
    ...
    let is_interacting = demo.interaction.grabbed_entity.is_some();
...
pub fn lean_system(demo: &mut LevelDemo, world: &mut World) {
    let Some(camera_entity) = demo.camera_entity else {
        return;
    };
...
pub fn crouch_camera_system(demo: &LevelDemo, world: &mut World) {
