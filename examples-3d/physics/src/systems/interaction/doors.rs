...
pub(super) fn update_manipulated_door(
    ...
    let Some(door_game_entity) = game_world
        .resources
        .interaction
        .manipulated_entity_of_kind(&crate::ecs::InteractableKind::Door)
    else {
        return;
    };
    ...
    if nalgebra_glm::distance(&camera_position, &hinge_position)
...
pub fn update_doors_momentum(game_world: &mut GameWorld, world: &mut World) {
    ...
    let manipulated_door = game_world
        .resources
        .interaction
        .manipulated_entity_of_kind(&crate::ecs::InteractableKind::Door);
