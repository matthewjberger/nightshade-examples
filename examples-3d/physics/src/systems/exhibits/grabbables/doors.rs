...
use crate::ecs::{DOOR, Door, GameWorld, INTERACTABLE, Interactable, InteractableKind};
use nightshade::ecs::physics::*;
use nightshade::prelude::*;

pub(crate) fn spawn_door_exhibit(game_world: &mut GameWorld, world: &mut World, center: Vec3) {
    let door_width = 0.9;
    let door_height = 2.5;
    ...
    if let Some(rigid_body) = world.core.get_rigid_body_mut(door_entity) {
        *rigid_body =
            RigidBodyComponent::new_kinematic().with_translation(door_center_x, frame_y, frame_z);
    }
