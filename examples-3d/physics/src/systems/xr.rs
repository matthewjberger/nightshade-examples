...
pub fn xr_hand_tracking_system(game_world: &mut GameWorld, world: &mut World) {
    ...
    {
        let aim = nalgebra_glm::quat_rotate_vec3(&aim_rot, &nalgebra_glm::vec3(0.0, 0.0, 1.0));
        let up = nalgebra_glm::quat_rotate_vec3(&aim_rot, &nalgebra_glm::vec3(0.0, 1.0, 0.0));
        let right = nalgebra_glm::cross(&aim, &up);

        let mat = nalgebra_glm::Mat3::new(
            right.x, up.x, -aim.x, right.y, up.y, -aim.y, right.z, up.z, -aim.z,
        );
