use crate::ecs::{GameWorld, INTERACTABLE, Interactable, InteractableKind, LEVER, Lever};
use nightshade::ecs::physics::*;
use nightshade::ecs::transform::components::Parent;
use nightshade::prelude::*;

pub(crate) fn spawn_lever_exhibit(game_world: &mut GameWorld, world: &mut World, center: Vec3) {
    ...
    let handle_material = create_textured_material(nalgebra_glm::vec3(0.15, 0.15, 0.17), 0.3, 0.7);
    ...
    let lever_entities: Vec<freecs::Entity> =
        game_world.query_entities(crate::ecs::LEVER).collect();
