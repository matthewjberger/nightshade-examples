...
pub(super) fn update_manipulated_wheel(
    ...
    let Some(wheel_game_entity) = game_world
        .resources
        .interaction
        .manipulated_entity_of_kind(&crate::ecs::InteractableKind::Wheel)
    else {
        return;
    };
    ...
    if nalgebra_glm::distance(&camera_position, &center_position)
...
pub fn update_wheels_momentum(game_world: &mut GameWorld, world: &mut World) {
    ...
    let wheel_entities: Vec<freecs::Entity> = game_world.query_entities(WHEEL).collect();

    let manipulated_wheel = game_world
        .resources
        .interaction
        .manipulated_entity_of_kind(&crate::ecs::InteractableKind::Wheel);
