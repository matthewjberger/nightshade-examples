...
impl Environment {
    ...
    pub fn sync_wolf_transform(&self, world: &mut World) {
        if let Some(transform) = world.get_local_transform_mut(self.wolf.body) {
            transform.translation = self.wolf.position;
        }
        world.set_local_transform_dirty(self.wolf.body, LocalTransformDirty);

        let is_night = self.day_night.is_night();
        let hunt_radius = self.wolf.hunt_radius(is_night);
        let diameter = hunt_radius * 2.0;
        if let Some(transform) = world.get_local_transform_mut(self.wolf.hunt_radius_entity) {
        ...
        world.set_local_transform_dirty(self.wolf.hunt_radius_entity, LocalTransformDirty);
