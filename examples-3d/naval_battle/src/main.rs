...
impl State for NavalBattle {
    ...
    fn initialize(&mut self, world: &mut World) {
        ...
        self.player_entity = Some(self.spawn_ship(world, vec3(0.0, 0.0, 0.0), 0.0, ShipFaction::Player));
        ...
        for (position, heading) in [(vec3(60.0, 0.0, 30.0), 2.5), (vec3(-50.0, 0.0, -40.0), 1.0), (vec3(20.0, 0.0, -70.0), 4.0)] {
        ...
        egui::Area::new(egui::Id::new("hud"))
            ...
            .show(ui_context, |ui| {
                egui::Frame::NONE
                    ...
                    .show(ui, |ui| {
                        if ready {
                            ui.label(egui::RichText::new("CANNONS READY").size(20.0).color(egui::Color32::from_rgb(0, 255, 80)));
                        } else {
                            ui.label(egui::RichText::new(format!("Reloading: {cooldown:.1}s")).size(20.0).color(egui::Color32::from_rgb(255, 160, 50)));
                        }
                        ui.add_space(4.0);
                        ui.label(egui::RichText::new(format!("Enemy ships: {enemies_alive}/{enemies_total}")).size(18.0).color(egui::Color32::WHITE));

                        ui.add_space(8.0);
                        let radar_size = 120.0;
                        let radar_range = 120.0;
                        let (response, painter) = ui.allocate_painter(egui::vec2(radar_size, radar_size), egui::Sense::hover());
                        let center = response.rect.center();

                        painter.circle_filled(center, radar_size / 2.0, egui::Color32::from_black_alpha(180));
                        painter.circle_stroke(center, radar_size / 2.0, egui::Stroke::new(1.0, egui::Color32::from_rgb(0, 100, 0)));
                        painter.circle_stroke(center, radar_size / 4.0, egui::Stroke::new(0.5, egui::Color32::from_rgb(0, 60, 0)));

                        let heading_sin = player_heading.sin();
                        let heading_cos = player_heading.cos();
                        let fwd_len = radar_size / 2.0 - 4.0;
                        painter.line_segment(
                            [center, egui::pos2(center.x - heading_sin * fwd_len * 0.3, center.y - heading_cos * fwd_len * 0.3)],
                        ...
                        for &enemy in &self.game.resources.enemy_list {
                            let Some(ship) = self.game.get_ship(enemy) else { continue };
                            ...
                            if dist > radar_size / 2.0 - 4.0 { continue }
                        ...
                        ui.add_space(6.0);
                        ui.label(egui::RichText::new("Q port | E starboard").size(13.0).color(egui::Color32::from_white_alpha(140)));
                        ui.label(egui::RichText::new("WA/D move/turn | Scroll zoom").size(13.0).color(egui::Color32::from_white_alpha(140)));
                    });
            });
                    ...
                    egui::Frame::NONE
                        ...
                        .show(ui, |ui| {
                            ui.vertical_centered(|ui| {
                                ui.label(egui::RichText::new("ALL ENEMIES SUNK!").size(48.0).color(egui::Color32::from_rgb(255, 217, 0)));
                            });
                        });
                });
        }
...
impl NavalBattle {
        ...
        match import_gltf_from_path(model_path) {
            Ok(result) => {
                for (name, (rgba_data, width, height)) in result.textures {
                    world.queue_command(WorldCommand::LoadTexture { name, rgba_data, width, height });
                }
    ...
    fn spawn_ship(&mut self, world: &mut World, position: Vec3, heading: f32, faction: ShipFaction) -> freecs::Entity {
        ...
        let root_entity = if let Some(prefab) = &self.player_prefab {
            ...
            if let Some(transform) = world.core.get_local_transform_mut(entity) {
                transform.scale = Vec3::new(PLAYER_SHIP_SCALE, PLAYER_SHIP_SCALE, PLAYER_SHIP_SCALE);
            }
            mark_local_transform_dirty(world, entity);
            entity
        } else {
            spawn_entity_with_material(world, "Cube", position, vec3(3.0, 1.0, 16.0), "ship_hull_fallback")
        };
        ...
        self.game.set_entity_handle(game_entity, EntityHandle(root_entity));
        self.game.set_position(game_entity, Position(position));
        self.game.set_ship(game_entity, Ship {
            faction, heading, speed: PLAYER_SPEED, cannon_cooldown: 0.0, root_entity,
            smooth_y: 0.0, smooth_pitch: 0.0, smooth_roll: 0.0,
            hits_taken: 0, dead: false, sink_timer: 0.0,
        });
    ...
    fn player_input_system(&mut self, world: &mut World, delta: f32) {
        ...
        let Some(mut ship) = self.game.get_ship(player).cloned() else { return };

        let keyboard = &world.resources.input.keyboard;
        let mut turn = 0.0;
        let mut throttle = 0.0;

        if keyboard.is_key_pressed(KeyCode::KeyA) || keyboard.is_key_pressed(KeyCode::ArrowLeft) { turn += 1.0 }
        if keyboard.is_key_pressed(KeyCode::KeyD) || keyboard.is_key_pressed(KeyCode::ArrowRight) { turn -= 1.0 }
        if keyboard.is_key_pressed(KeyCode::KeyW) || keyboard.is_key_pressed(KeyCode::ArrowUp) { throttle += 1.0 }
        ...
        let position = self.game.get_position(player).map_or(Vec3::zeros(), |p| p.0);
        let new_position = position + forward * throttle * ship.speed * delta;
        if let Some(pos) = self.game.get_position_mut(player) { pos.0 = new_position }
        ...
        if ship.cannon_cooldown <= 0.0 {
            let fired = if fire_left {
                ...
                self.fire_cannonball(world, new_position + side * 4.0 + vec3(0.0, 4.0, 0.0), vec3(side.x, 0.3, side.z));
                true
            } else if fire_right {
                let side = vec3(heading.cos(), 0.0, -heading.sin());
                self.fire_cannonball(world, new_position + side * 4.0 + vec3(0.0, 4.0, 0.0), vec3(side.x, 0.3, side.z));
            ...
            if fired { ship.cannon_cooldown = CANNON_COOLDOWN }
        }
    ...
    fn fire_cannonball(&mut self, world: &mut World, position: Vec3, direction: Vec3) {
        let game_entity = self.game.spawn_entities(ENTITY_HANDLE | POSITION | CANNONBALL, 1)[0];
        let velocity = direction.normalize() * CANNONBALL_SPEED;

        let ball_entity = spawn_entity_with_material(world, "Sphere", position, vec3(0.8, 0.8, 0.8), "cannonball_glow");

        let trail_emitter = spawn_particle_emitter(world, position, ParticleEmitter {
        ...
        self.game.set_entity_handle(game_entity, EntityHandle(ball_entity));
        self.game.set_position(game_entity, Position(position));
        self.game.set_cannonball(game_entity, CannonballComp { velocity, age: 0.0, trail_emitter });
    ...
    fn cannonball_system(&mut self, world: &mut World, delta: f32) {
        ...
        for &ball_entity in &cannonball_list {
            ...
            let position = self.game.get_position(ball_entity).map_or(Vec3::zeros(), |p| p.0);
            let mut new_velocity = ball.velocity;
            new_velocity.y -= CANNONBALL_GRAVITY * delta;
            let new_position = position + new_velocity * delta;

            if let Some(pos) = self.game.get_position_mut(ball_entity) { pos.0 = new_position }
    ...
    fn check_enemy_hit(&self, ball_pos: Vec3) -> Option<freecs::Entity> {
        ...
        for &enemy in &self.game.resources.enemy_list {
            let Some(ship) = self.game.get_ship(enemy) else { continue };
            if ship.dead || ship.faction != ShipFaction::Enemy { continue }
            let enemy_pos = self.game.get_position(enemy).map_or(Vec3::zeros(), |p| p.0);
    ...
    fn register_hit(&mut self, world: &mut World, enemy: freecs::Entity, hit_pos: Vec3) {
        let Some(ship) = self.game.get_ship(enemy).cloned() else { return };

        let marker = spawn_entity_with_material(world, "Cube", hit_pos, vec3(0.5, 0.5, 0.5), "hit_marker");
        parent_to(world, marker, ship.root_entity);

        let smoke = spawn_particle_emitter(world, hit_pos, ParticleEmitter {
            emitter_type: EmitterType::Smoke,
        ...
        let Some(ship_mut) = self.game.get_ship_mut(enemy) else { return };
    ...
    fn sinking_system(&mut self, world: &mut World, delta: f32) {
        ...
        for &enemy in &self.game.resources.enemy_list.clone() {
            let Some(mut ship) = self.game.get_ship(enemy).cloned() else { continue };
    ...
    fn spawn_explosion(&mut self, world: &mut World, position: Vec3) {
        let game_entity = self.game.spawn_entities(ENTITY_HANDLE | POSITION | EFFECT, 1)[0];
        let emitter_entity = spawn_particle_emitter(world, position, ParticleEmitter {
        ...
        self.game.set_entity_handle(game_entity, EntityHandle(emitter_entity));
        self.game.set_position(game_entity, Position(position));
        self.game.set_effect(game_entity, Effect { lifetime: 1.5, age: 0.0 });
    ...
    fn ship_bob_system(&mut self, world: &mut World, time: f32) {
        ...
        for &ship_entity in &all_ships.clone() {
            let Some(mut ship) = self.game.get_ship(ship_entity).cloned() else { continue };
            if ship.dead { continue }

            let position = self.game.get_position(ship_entity).map_or(Vec3::zeros(), |p| p.0);
            ...
            let height_bow = sample_wave_height(&self.water_config, position.x - heading_sin * SHIP_HALF_LENGTH, position.z - heading_cos * SHIP_HALF_LENGTH, time);
            let height_stern = sample_wave_height(&self.water_config, position.x + heading_sin * SHIP_HALF_LENGTH, position.z + heading_cos * SHIP_HALF_LENGTH, time);
            let height_port = sample_wave_height(&self.water_config, position.x - heading_cos * SHIP_HALF_BEAM, position.z + heading_sin * SHIP_HALF_BEAM, time);
            let height_starboard = sample_wave_height(&self.water_config, position.x + heading_cos * SHIP_HALF_BEAM, position.z - heading_sin * SHIP_HALF_BEAM, time);
            let height_center = sample_wave_height(&self.water_config, position.x, position.z, time);

            let target_y = (height_bow + height_stern + height_port + height_starboard + height_center) / 5.0 + SHIP_BOB_OFFSET;
    ...
    fn update_trajectory_lines(&self, world: &mut World) {
        let Some(player) = self.player_entity else { return };
        let Some(ship) = self.game.get_ship(player) else { return };
        let position = self.game.get_position(player).map_or(Vec3::zeros(), |p| p.0);
        let heading = ship.heading;
        ...
        if let Some(entity) = self.trajectory_line_port {
            let lines = compute_trajectory_arc(position + port_side * 4.0 + vec3(0.0, 4.0, 0.0), vec3(port_side.x, 0.3, port_side.z));
        ...
        if let Some(entity) = self.trajectory_line_starboard {
            let lines = compute_trajectory_arc(position + starboard_side * 4.0 + vec3(0.0, 4.0, 0.0), vec3(starboard_side.x, 0.3, starboard_side.z));
            if let Some(lines_comp) = world.core.get_lines_mut(entity) {
                lines_comp.lines = lines;
                lines_comp.version += 1;
            }
    ...
    fn camera_follow_system(&mut self, world: &mut World) {
        let Some(player) = self.player_entity else { return };
        let Some(camera) = self.camera_entity else { return };
        let position = self.game.get_position(player).map_or(Vec3::zeros(), |p| p.0);
    ...
    fn spawn_splash(&mut self, world: &mut World, position: Vec3) {
        ...
        self.game.set_entity_handle(game_entity, EntityHandle(emitter_entity));
        self.game.set_position(game_entity, Position(position));
        self.game.set_effect(game_entity, Effect { lifetime: 1.0, age: 0.0 });
        self.game.resources.effect_list.push(game_entity);
    }
    ...
    fn effect_system(&mut self, world: &mut World, delta: f32) {
        ...
        for &effect_entity in &effect_list {
            ...
            effect.age += delta;
            if effect.age >= effect.lifetime { to_remove.push(effect_entity) }
        }
        for entity in &to_remove {
            if let Some(handle) = self.game.get_entity_handle(*entity) {
                despawn_recursive_immediate(world, handle.0);
...
fn tint_prefab_red(world: &mut World, root: Entity) {
    let entities: Vec<Entity> = world.core.query_entities(RENDER_MESH | MATERIAL_REF).collect();
    for entity in entities {
        if entity != root && !is_descendant_of(world, entity, root) { continue }
        let material_name = format!("enemy_tint_{}", entity.id);
        material_registry_insert(
            &mut world.resources.material_registry, material_name.clone(),
            Material { base_color: [0.6, 0.1, 0.1, 1.0], roughness: 0.7, metallic: 0.1, ..Default::default() },
        );
...
fn is_descendant_of(world: &World, entity: Entity, ancestor: Entity) -> bool {
    ...
    for _ in 0..32 {
        ...
        {
            if parent_entity == ancestor { return true }
...
fn parent_to(world: &mut World, child: Entity, parent: Entity) {
    world.core.add_components(child, nightshade::ecs::world::PARENT);
...
fn spawn_camera(world: &mut World) -> Entity {
    ...
    let camera = spawn_pan_orbit_camera(world, vec3(0.0, 2.0, 0.0), 45.0, 0.0, 0.4, "Main Camera".to_string());
    if let Some(cam) = world.core.get_camera_mut(camera) {
        cam.projection = Projection::Perspective(PerspectiveCamera { aspect_ratio: None, y_fov_rad: 60.0_f32.to_radians(), z_far: Some(2000.0), z_near: 0.1 });
    }
    camera
}

fn spawn_ocean_sun(world: &mut World) {
    let sun = world.spawn_entities(NAME | LOCAL_TRANSFORM | LOCAL_TRANSFORM_DIRTY | GLOBAL_TRANSFORM | LIGHT, 1)[0];
    world.core.set_name(sun, Name("Sun".to_string()));
    world.core.set_local_transform(sun, LocalTransform { translation: Vec3::new(100.0, 80.0, 50.0), rotation: Quat::identity(), scale: Vec3::new(1.0, 1.0, 1.0) });
    world.core.set_local_transform_dirty(sun, LocalTransformDirty);
    world.core.set_global_transform(sun, GlobalTransform::default());
    world.core.set_light(sun, Light { light_type: LightType::Directional, color: Vec3::new(1.0, 0.92, 0.75), intensity: 3.5, range: 0.0, inner_cone_angle: 0.0, outer_cone_angle: 0.0, cast_shadows: true, shadow_bias: 0.005 });
    if let Some(transform) = world.core.get_local_transform_mut(sun) {
        ...
        transform.rotation = nalgebra_glm::mat3_to_quat(&nalgebra_glm::mat3(right.x, cup.x, fwd.x, right.y, cup.y, fwd.y, right.z, cup.z, fwd.z));
    }
...
fn spawn_ocean(world: &mut World) {
    ...
    world.core.set_water(entity, Water { base_color: [0.0, 0.05, 0.12, 1.0], water_color: [0.1, 0.25, 0.35, 1.0], wave_height: 0.6, choppy: 4.0, speed: 0.8, frequency: 0.16, ..Default::default() });
}

fn register_materials(world: &mut World) {
    register_material(world, "ship_hull_fallback", [0.35, 0.20, 0.10, 1.0], 0.8, 0.0, [0.0; 3]);
    register_material(world, "cannonball_glow", [0.15, 0.12, 0.1, 1.0], 0.4, 0.8, [0.0; 3]);
    register_material(world, "hit_marker", [0.8, 0.05, 0.05, 1.0], 0.9, 0.0, [0.5, 0.0, 0.0]);
}

fn register_material(world: &mut World, name: &str, color: [f32; 4], roughness: f32, metallic: f32, emissive: [f32; 3]) {
    material_registry_insert(&mut world.resources.material_registry, name.to_string(), Material {
        base_color: color, roughness, metallic, emissive_factor: emissive, ..Default::default()
    });
    if let Some(&index) = world.resources.material_registry.registry.name_to_index.get(name) {
        world.resources.material_registry.registry.add_reference(index);
    }
}

fn spawn_trajectory_lines(world: &mut World) -> Entity {
    let entity = world.spawn_entities(NAME | LOCAL_TRANSFORM | LOCAL_TRANSFORM_DIRTY | GLOBAL_TRANSFORM | nightshade::ecs::world::LINES, 1)[0];
    world.core.set_name(entity, Name("Trajectory".to_string()));
    world.core.set_local_transform(entity, LocalTransform { translation: Vec3::zeros(), rotation: Quat::identity(), scale: Vec3::new(1.0, 1.0, 1.0) });
...
fn compute_trajectory_arc(origin: Vec3, direction: Vec3) -> Vec<Line> {
    ...
    for _ in 0..steps {
        ...
        pos += vel * dt;
        if pos.y < 0.0 { break }
        lines.push(Line { start: prev, end: pos, color });
    }
    lines
}

fn spawn_entity_with_material(world: &mut World, mesh_name: &str, position: Vec3, scale: Vec3, material_name: &str) -> Entity {
...
fn spawn_particle_emitter(world: &mut World, position: Vec3, emitter: ParticleEmitter) -> Entity {
    let entity = world.spawn_entities(NAME | LOCAL_TRANSFORM | LOCAL_TRANSFORM_DIRTY | GLOBAL_TRANSFORM | nightshade::ecs::world::PARTICLE_EMITTER, 1)[0];
    world.core.set_name(entity, Name("Particle".to_string()));
    world.core.set_local_transform(entity, LocalTransform { translation: position, rotation: Quat::identity(), scale: Vec3::new(1.0, 1.0, 1.0) });
    world.core.set_local_transform_dirty(entity, LocalTransformDirty);
    world.core.set_global_transform(entity, GlobalTransform::default());
