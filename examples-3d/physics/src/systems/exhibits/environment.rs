...
pub fn spawn_sun_overhead(world: &mut World) -> Entity {
    ...
    world.core.set_name(entity, Name("Sun".to_string()));
...
pub fn spawn_environment(world: &mut World) {
    let floor_material = create_textured_material(nalgebra_glm::vec3(0.15, 0.15, 0.18), 0.9, 0.0);
    let wall_material = create_textured_material(nalgebra_glm::vec3(0.2, 0.18, 0.16), 0.95, 0.0);
    let platform_material =
        create_textured_material(nalgebra_glm::vec3(0.22, 0.22, 0.25), 0.85, 0.1);
    let accent_material = create_textured_material(nalgebra_glm::vec3(0.3, 0.25, 0.2), 0.8, 0.0);
