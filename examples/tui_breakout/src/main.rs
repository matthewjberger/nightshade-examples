    ...
    fn check_ball_collisions(&mut self, world: &mut World) {
        ...
        for contact in &contacts {
            ...
            if self.wall_entities.contains(&other) {
                ...
                if let (Some(_ball_pos), Some(wall_pos)) = (ball_pos, wall_pos) {
                    ...
                    if let Some(position) = world.get_position_mut(self.ball_entity) {
                        position.column =
                            (position.column as i32 + self.ball_velocity_column) as f64;
                        position.row = (position.row as i32 + self.ball_velocity_row) as f64;
                    }
                }
                continue;
            }
