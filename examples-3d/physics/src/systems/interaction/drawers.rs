...
pub(super) fn update_manipulated_drawer(
    ...
    let Some(drawer_game_entity) = game_world
        .resources
        .interaction
        .manipulated_entity_of_kind(&crate::ecs::InteractableKind::Drawer)
    else {
        return;
    };

    let Some(drawer_info) = game_world
        .get_drawer(drawer_game_entity)
        .map(|drawer| (drawer.closed_position, drawer.current_offset))
    else {
        return;
    };
...
pub fn update_drawers_momentum(game_world: &mut GameWorld, world: &mut World) {
    ...
    let drawer_entities: Vec<freecs::Entity> = game_world.query_entities(DRAWER).collect();

    let manipulated_drawer = game_world
        .resources
        .interaction
        .manipulated_entity_of_kind(&crate::ecs::InteractableKind::Drawer);
