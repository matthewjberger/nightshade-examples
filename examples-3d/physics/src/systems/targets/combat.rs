use crate::ecs::{GameWorld, Health, SHOT_BAUBLE, TARGET, TargetKilledEvent};
...
pub(crate) fn update_targets(game_world: &mut GameWorld, world: &mut World) {
    ...
    for game_entity in target_entities {
        ...
        for &(bauble_game_entity, bauble_engine_entity, bauble_position) in &baubles {
            if consumed_baubles
                .iter()
                .any(|(ge, _)| *ge == bauble_game_entity)
            {
                continue;
            }
            ...
            && !target.popped
        {
            ...
            if let Some(transform) = world.core.get_local_transform_mut(target.health.bar_entity) {
