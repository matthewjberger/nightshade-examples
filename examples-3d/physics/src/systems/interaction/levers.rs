...
pub(super) fn update_manipulated_lever(
    ...
    let Some(lever_game_entity) = game_world
        .resources
        .interaction
        .manipulated_entity_of_kind(&crate::ecs::InteractableKind::Lever)
    else {
        return;
    };
    ...
    if nalgebra_glm::distance(&camera_position, &pivot_position)
        > game_world.resources.config.interact_range * 3.0
    {
...
pub fn update_levers_momentum(game_world: &mut GameWorld, world: &mut World) {
    ...
    let lever_entities: Vec<freecs::Entity> = game_world.query_entities(LEVER).collect();

    let manipulated_lever = game_world
        .resources
        .interaction
        .manipulated_entity_of_kind(&crate::ecs::InteractableKind::Lever);
