...
use crate::replay::{ReplayAction, take_snapshot};
...
pub fn end_turn(game_world: &mut GameWorld, events: &mut GameEvents) -> TurnTransition {
    ...
    let snapshot_index = events.replay_actions.len();
    events.replay_actions.push(ReplayAction::TurnStart {
        faction: next,
        turn: game_world.resources.turn_number,
    });
    let snapshot = take_snapshot(game_world);
    events.replay_snapshots.push((snapshot_index, snapshot));
