...
impl Survivors {
    ...
    fn spawn_player(&mut self, world: &mut World) {
        ...
        let axe_handle = world.spawn_entities(
            LOCAL_TRANSFORM
        ...
        world.set_local_transform(
            axe_handle,
            LocalTransform {
                translation: Vec3::new(PLAYER_RADIUS + 0.3, 0.0, 0.0),
                rotation: nalgebra_glm::quat_angle_axis(std::f32::consts::FRAC_PI_2, &Vec3::z()),
                scale: Vec3::new(0.06, 0.8, 0.06),
            },
        );
        world.set_render_mesh(axe_handle, RenderMesh::new("Cylinder"));
        world.set_parent(axe_handle, Parent(Some(player)));

        let axe_handle_material = format!("AxeHandle_{}", axe_handle.id);
        material_registry_insert(
            &mut world.resources.material_registry,
            axe_handle_material.clone(),
            Material {
        ...
        self.apply_material(world, axe_handle, &axe_handle_material);

        let axe_head = world.spawn_entities(
            LOCAL_TRANSFORM
                | LOCAL_TRANSFORM_DIRTY
                | GLOBAL_TRANSFORM
                | RENDER_MESH
        ...
        world.set_local_transform(
            axe_head,
            LocalTransform {
                translation: Vec3::new(0.0, 0.35, 0.15),
                rotation: Quat::identity(),
                scale: Vec3::new(0.08, 0.3, 0.5),
            },
        );
        world.set_render_mesh(axe_head, RenderMesh::new("Cube"));
        world.set_parent(axe_head, Parent(Some(axe_handle)));

        let axe_head_material = format!("AxeHead_{}", axe_head.id);
        material_registry_insert(
            &mut world.resources.material_registry,
            axe_head_material.clone(),
            Material {
                base_color: [0.7, 0.7, 0.75, 1.0],
                roughness: 0.2,
                metallic: 0.95,
                emissive_factor: [0.1, 0.1, 0.12],
                ..Default::default()
            },
        );
        self.apply_material(world, axe_head, &axe_head_material);

        self.axe_entity = Some(axe_handle);
    }
