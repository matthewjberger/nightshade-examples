...
pub fn App() -> impl IntoView {
    ...
    let thinking_started_at = RwSignal::new(Option::<f64>::None);
        ...
        nightshade::webview::connect(FrontendCommand::Ready, move |event| {
            match event {
                ...
                BackendEvent::StatusUpdate { status: agent_status } => {
                    if matches!(agent_status, AgentStatus::Thinking) && thinking_started_at.get_untracked().is_none() {
                        thinking_started_at.set(Some(js_sys::Date::now()));
                    }
