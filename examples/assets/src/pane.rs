                ...
                for row in 0..rows {
                    ui.horizontal(|ui| {
                        ...
                        for col in 0..columns {
                            ...
                            } else if let Some(handle) = self.thumbnail_cache.get(asset_path) {
                                Self::paint_thumbnail_image(ui, handle, thumb_rect, thumbnail_size);
                            }
                            ...
                            let display_name = if char_count > max_chars {
                                let truncated: String =
                                    filename.chars().take(max_chars.saturating_sub(3)).collect();
                                format!("{}...", truncated)
                            } else {
                                filename.clone()
                            };
